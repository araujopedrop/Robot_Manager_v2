...
AndinoCommand::AndinoCommand() : Node("andino_command")
{
...
void AndinoCommand::start_mapping_cb(const std::shared_ptr<std_srvs::srv::Trigger::Request>,
...
void AndinoCommand::stop_mapping_cb(const std::shared_ptr<std_srvs::srv::Trigger::Request>,
                                    std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
    if (mapping_pid_ <= 0) {
        response->success = false;
...
void AndinoCommand::start_saver_cb(const std::shared_ptr<andino_custom_interfaces::srv::SaveMap::Request> request,
...
void AndinoCommand::stop_saver_cb(const std::shared_ptr<std_srvs::srv::Trigger::Request>,
                                  std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
    if (saver_pid_ <= 0) {
        response->success = false;
...
void AndinoCommand::delete_map_cb(const std::shared_ptr<andino_custom_interfaces::srv::SaveMap::Request> request,
                                  std::shared_ptr<andino_custom_interfaces::srv::SaveMap::Response> response)
{
    std::string mapa = request->nombre;
    std::string base_path = this->path_maps + mapa;
...
void AndinoCommand::start_and_finalize_saver_cb(const std::shared_ptr<andino_custom_interfaces::srv::SaveMap::Request> request,
                                                std::shared_ptr<andino_custom_interfaces::srv::SaveMap::Response> response)
{
    int status;
...
bool AndinoCommand::mapWasCreated(const std::string& folder, const std::string& fileName)
{
...
void AndinoCommand::finalize_processes()
{
    if (mapping_pid_ > 0) {
        kill(-mapping_pid_, SIGTERM);
        waitpid(mapping_pid_, nullptr, 0);
